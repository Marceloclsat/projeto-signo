        ...
        if ($data_inicial_signo > $data_final_signo) {
            if ($data_nasc_formated >= $data_inicial_signo || $data_nasc_formated <= $data_final_signo) {
                return [
                    ...
                    'signoemoji' => (string ) $signo->signoemoji
                ];
            }
        } else {
            if ($data_nasc_formated >= $data_inicial_signo && $data_nasc_formated <= $data_final_signo) {
...
function renderizarResultado($resultado){
    echo "<body>
        <div class='d-flex flex-column vh-100 text-white'>
            <div class='w-100 h-100 text-center p-5 d-flex flex-column justify-content-around'>
                <h1 class='display-1 d-block nome-signo'>{$resultado['signo']}</h1>
                <p class='display-2'>&#x{$resultado['signoemoji']}</p>
